...
exports.default = async conf => {
  const mcVersions = await (0, _getVersions2.default)(conf.JAR_version_url, 'minecraft');
  const forgeVersions = await (0, _getVersions2.default)(conf.JAR_forge_version_url, 'forge');
  ...
  let questions = [{
    ...
    choices: mcVersions.map(le => le.id).concat([new _choicesSeparator2.default()]),
    prefix: pager()
  }];

  const forgeVersionPage = pager();
  (0, _lodash.forEach)((0, _lodash.groupBy)(forgeVersions, v => v.dep), (le, k) => {
    questions.push({
      when: an => an.MCINTERNAL_version === k,
      type: 'list',
      name: 'FORGE_version',
      message: 'Which version of Forge ?',
      choices: [''].concat(le.map(info => info.id)).concat([new _choicesSeparator2.default()]),
      filter: v => v || false,
